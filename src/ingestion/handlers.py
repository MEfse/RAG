...
class CSVChunkReader:
    def __init__(self, default_encoding: str = "latin-1") -> None:
        """
    ...
    def load_csv(
        ...
        try:           
            data = pd.read_csv(path, encoding=encoding, nrows=nrows, chunksize=chunksize)
            
            logger.info(f"Файл {path} открыт в режиме чанков (chunksize={chunksize})")
